...
if (isset($_POST['save'])) { // if save button on the form is clicked
   ...
   $user = $_POST['email'];
    ...
    if (!in_array($extension, ['pdf' ,'xlsx','ppt'])) {
        echo '<script type = "text/javascript">
                    alert("You file extension must be:  .pdf , .xlsx , .ppt");
                    window.location = "add_file.php";
                </script>
';
        // echo "<h6 style='color:red'>You file extension must be .ppt .xlsx.pdf/h6>";
...
//            $user = $_SESSION['email_address'];
        ...
        date_default_timezone_set("Asia/Kolkota");
         $time = date("M-d-Y h:i s",strtotime("+0 HOURS"));
